...
class BasePolicy(BaseModel, ABC):
    ...
    def predict(
        ...
        #여기 바꿈
        actions[[12,13,14]] = 0 #waist
        actions[[16,17,19,20,21]] = 0 #left arm
        actions[[23,24,26,27,28]] = 0 #right arm
...
class ActorCriticPolicy(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        # fixed_actions  = th.tensor(fixed_actions , dtype=th.float32, device='cuda:0')
        # fixed_actions = fixed_actions.unsqueeze(0).expand(actions.shape[0], -1)
        # noise = th.randn_like(fixed_actions) * 0.05

        # actions = fixed_actions + noise
